...
$(document).ready(function (){
	...
	$('#year-month').text(c_year+"년 "+c_month+"월");
...
$(".m-row").click(function(){
	var idx = $(".m-row").index(this);
	var this_id = $(this).children()[1].id;
	var time_arr = get_each_time(get_time_from_tr(timerange, tr_mode), date_stat);
	$("#modal-"+this_id+"-hour").text(time_arr[(idx+1)%5]);
	if(~this_id.indexOf("we")){
		// weekends
		$("#modal-"+this_id+"-day").text(date_stat[1]);
	} else {
		// weekdays
		$("#modal-"+this_id+"-day").text(date_stat[0]);
	}
});
